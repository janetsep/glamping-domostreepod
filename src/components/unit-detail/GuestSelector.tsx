...
export const GuestSelector = ({
  ...
  return <div className="w-full">
      <label htmlFor="guest-selector" className="block text-sm font-medium text-gray-700 mb-3">
      ...
      <div className="border border-gray-300 rounded-lg p-4 space-y-4 bg-white">
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-gray-700">Número de huépedes:</span>
          <div className="flex items-center space-x-3">
            <Button type="button" variant="outline" size="sm" className="w-8 h-8 p-0 rounded-full" onClick={() => handleAdultsChange(adults - 1)} disabled={adults <= 0}>
              <Minus className="w-4 h-4" />
            </Button>
            <span className="w-8 text-center font-medium">{adults}</span>
            <Button type="button" variant="outline" size="sm" className="w-8 h-8 p-0 rounded-full" onClick={() => handleAdultsChange(adults + 1)} disabled={adults + children >= maxAllowedGuests}>
      ...
      {availableDomos !== undefined && maxAllowedGuests < maxGuests && availableDomos > 0 && <p className="text-amber-600 text-sm mt-2">
          Solo hay {availableDomos} {availableDomos === 1 ? 'domo disponible' : 'domos disponibles'} 
          (máximo {maxAllowedGuests} huéspedes).
        </p>}
      
      {availableDomos !== undefined && availableDomos === 0 && <p className="text-red-600 text-sm mt-2">
          No hay domos disponibles para las fechas seleccionadas.
        </p>}
      
      
      {value > 0 && <p className="text-blue-700 text-sm mt-2 font-medium">
