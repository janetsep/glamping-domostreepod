  ...
  return <section id="packages" className="py-10 bg-secondary/10">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {travelerTypesContent.travelerTypes.map(type => <div key={type.id} className="bg-white rounded-lg overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl" onMouseEnter={() => setHoveredType(type.id)} onMouseLeave={() => setHoveredType(null)} onClick={() => handleTravelerTypeClick(type.id)}>
              <div className="relative h-64 cursor-pointer">
                <img src={type.image} alt={type.name} className="w-full h-full object-cover transition-transform duration-500 ease-in-out transform hover:scale-105" />
